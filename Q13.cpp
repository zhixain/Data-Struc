...
public:
    ...
    friend std::ostream &operator<<(std::ostream &out, Node n) {//用值傳遞
        out<<*(n.data);
        return out;
    }
    friend std::ostream &operator<<(std::ostream &out, Node *n) {//用ptr傳遞
        out<<*(n->data);
        return out;
    }
    void setData(T d) {
    ...
    TreeNode<T> *operator[](int n) {
        ...
        while (!q.empty()) {
              ...
              for (int i = 0; i < current -> size(); i++) {
                q.push((*current)[i]);//current的child node
              }
              k++;
        }
        return NULL;
